...
#Example of how each county would go into the function
#current_county = "Albany County"

#### Function is wrapped around all of the code from here down 
run_model <- function(current_county){
  df = df %>% 
    arrange( week, country, state, key, lat, long, county,population) %>% 
    filter(Recip_County == current_county)
    ...
    while(i > 0){
      if(!converged){
        ...
        for (mc in (1:3)) {
          ...
          try(beta_covar_training <- chol2inv(chol( (t(X_mat_pre_changepoint_training) %*% 
                                                  X_mat_pre_changepoint_training)/sigma_eps_sq_training)),
                break) #+ diag(p+1)/10^4
          
          try(beta_star_covar_training <- chol2inv(chol( (t(X_mat_post_changepoint_training) %*% 
...
##----------------------------------------------------------------------------------------------
#List of counties we want to run it on
county_list <- c("Albany County", "Allegany County", "Bronx County", "Broome County", "Cattaraugus County", "Cayuga County",
                 "Chautauqua County", "Chemung County", "Chenango County" , "Clinton County")

#Running them with parallel processing
#This function should run the model (from the function created) on each of the counties in the list at the same time. 
#The function doesn't output anything itself, but within the function it should save the needed parameters to a .RData file
mclapply(county_list, run_model, mc.cores = 5)
