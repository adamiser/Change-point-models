...
county_list <- c("Columbia County", "Cortland County", "Delaware County", "Dutchess County",
                 "Erie County", "Essex County", "Franklin County", "Fulton County", "Genesee County",
                 "Greene County")

#### For loop is wrapped around entire model
      ...
      if(!converged){
        ...
        for (mc in (1:3)) {
          ...
          for (tmp in (1:n_tmp_training)){
            ...
            #Can be made faster by doing this for all change points or instead of doing it for all check if for
            #the change point this is already done if so get the value from there
            try(mu_cond_training <- ((t(SIGMA_u_tmp_trans_21_mat_training[,tmp]) %*%
          ...
          try(beta_mean_training <- beta_covar_training %*% ( t(X_mat_pre_changepoint_training)%*% 
                                                           (pi_pre_changepoint_training - u_minus_training))/sigma_eps_sq_training,
              break)
          
          try(beta_star_mean_training <- (beta_star_covar_training %*% (t(X_mat_post_changepoint_training)%*% 
